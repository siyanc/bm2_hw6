...
summary(glm_fit)
```
Yes, predictors Lag1, Lag2 appear to be statistically significant.
...
```
From the confusion matrix, sensiticity is 0.8146 (the probability of true "Up" is 0.8146 when the direction is predicted to be Up) and the sensitivity is 0.1322 (the probability of not "Up" is 0.1322 when the direction is predicted to be down)
Accuracy tells the overall probaility of correct classifier is 0.511.
Kappa is negative, which means that there is less agrrement than would be expected by chance give the marginal distribution. 
...
Based on the plot, the AUC us 0.522, which suggests that the capability of logistic models distinguishing between classes is not good. 
